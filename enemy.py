...
class Enemy(Entity):
    def __init__(self, monster_name: str, monster_type: str, game_difficulty: int, game_round: int, map_size: int, pos: tuple, groups: list, obstacle_sprites: pygame.sprite.Group, damage_player: Callable, trigger_death_particles: Callable, monster_count_down: Callable, drop_item: Callable, add_exp: Callable, drop_weapon: Callable) -> None:
        # general setup
        ...
        self.health        = (monster_info['health'] + monster_info['health'] * (game_round / 5)) * game_difficulty
        self.max_health = self.health
        if game_difficulty == 2: # 하드 모드 경험치, 몬스터 데미지 배율 2
            rate = 2
        else: # 이지 노말 난이도 경험치, 몬스터 데미지 배율 1
            rate = 1
        print(rate)
        self.exp           = (monster_info['exp'] + monster_info['exp'] * (game_round / 5)) * rate
        self.speed         = monster_info['speed']
        self.attack_damage = (monster_info['damage'] + monster_info['damage'] * (game_round / 5)) * rate
